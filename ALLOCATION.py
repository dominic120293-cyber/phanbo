...
SOLVER_TIME_LIMIT = 50     # giây – 0 clashes thường tìm thấy trước 40s
...
def run_optimization(file_input):
    ...
    # ── Pre-compute style IDs (dùng dummy cells → lấy integer style ID) ──
    # openpyxl lưu style dưới dạng integer index nội bộ.
    # Gán ._style trực tiếp nhanh hơn set font/fill/border ~4× vì bỏ qua validation.
    ...
    def write_result_sheet(ws, df, sheet_title):
        ...
        # ── Pass 1: bulk-append giá trị + thu thập style key từng dòng ──
        data_sids = []
        ...
        # ── Pass 2: áp style bằng ._style integer (nhanh ~4× so với set font/fill) ──
        ...
        # ── Sheet default row height (thay per-row ws.row_dimensions) ──
        # Tránh tạo XML element cho từng dòng → tiết kiệm ~15s với 832 rows
